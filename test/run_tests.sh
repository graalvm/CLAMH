...
# Runs all of the tests in the clamh/test directory:
# (This is a rudimentary initial version and will be expanded upon later)
...
echo "Building C++..."
clamh_build_cpp.sh factorial.cpp || die "C++ build failed"

echo "Building Java..."
clamh_build_java.sh Factorial.java || die "Java build failed"

echo "Executing clamh_exec_all.sh..."
../../sample_scripts/clamh_exec_all.sh --cpp=run_factorial --java=run_Factorial.jar -o test1 || die "clamh_exec_all.sh failed"
